import React, { useState } from 'react'
import { MdClose, MdOutlineDateRange } from "react-icons/md";
import moment from "moment"
import { DayPicker } from "@daypicker/react";


const DateSelector = ({date,setDate}) => {

    const [openDatePicker,setOpenDatePicker] = useState(false)

  return (
    <div>

        <button className=' inline-flex items-center gap-3 text-[13px] font-medium text-sky-600 bg-sky-100 hover:bg-sky-300 rounded px-2 py-1 cursor-pointer  ' onClick={()=>{setOpenDatePicker(true)}}>
            <MdOutlineDateRange />
            {date? moment(date).format("Do MMM YYYY") : moment().format("Do MMM YYYY") }
        </button>

       { openDatePicker &&  <div className='overflow-y-scroll p-5 bg-sky-200 rounded-lg relative pt-9'>

            <button className='w-10 h-10 rounded-full flex items-center justify-center bg-sky-100 hover:bg-sky-100 absolute top-2 right-3 cursor-pointer hover:bg-sky-400 ' onClick={()=>{setOpenDatePicker(false)}} >
                <MdClose className='text-xl text-sky-600 hover:text-black' />               
            </button>

            <DayPicker 
                captionLayout='dropdown-button'
                mode="single"
                selected={date}
                onSelect={setDate}
            />


        </div>}

    </div>
  )
}

export default DateSelector